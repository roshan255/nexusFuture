...
class Settings:
    ...
    min_24h_abs_change_percent: float = 3.0; min_short_term_move_percent: float = 0.25
...
def load_settings() -> Settings:
    ...
    return Settings(mode=mode,api_key=g("BINANCE_API_KEY"),api_secret=g("BINANCE_API_SECRET"),leverage=int(g("LEVERAGE","5")),margin_per_trade_usdt=float(g("MARGIN_PER_TRADE_USDT","10")),margin_mode=margin_mode,margin_percent=percent,take_profit_percent=float(g("TAKE_PROFIT_PERCENT",".50")),stop_loss_percent=float(g("STOP_LOSS_PERCENT",".30")),min_score=float(g("MIN_SCORE","70")),min_direction_gap=float(g("MIN_DIRECTION_GAP","5")),min_24h_abs_change_percent=float(g("MIN_24H_ABS_CHANGE_PERCENT","3")),min_short_term_move_percent=float(g("MIN_SHORT_TERM_MOVE_PERCENT","0.25")),pre_filter_limit=int(g("PRE_FILTER_LIMIT","50")),deep_analysis_limit=int(g("DEEP_ANALYSIS_LIMIT","20")),enable_live_trading=_bool(g("ENABLE_LIVE_TRADING","false")),uat_base_url=g("UAT_BASE_URL","https://demo-fapi.binance.com"))
